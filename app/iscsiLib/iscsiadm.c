           ...
           int info_level, int do_login, int op)
{
    ...
    /*获取路径名*/
    if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),IFACE_CONFIG_DIR))   
    {
        return ENOTDIR;
    }
...
int main(int argc ,char* argv[])
{
    ...
    char  *group_session_mgmt_mode = NULL;
    int ch, longindex, do_rescan=0;
    int rc=0, sid=-1, op=OP_NOOP, do_stats=0;
    ...
    int do_login = 0;
    int do_logout = 0;   
    char inname[TARGET_NAME_MAXLEN];
    char * ip = address; 
    char *targetname = inname ;

    memset(address,0,sizeof(address));
    memset(inname,0,sizeof(inname));
    if(argc < 8)
    {
        printf("iscsiadm main arg error\n");
        return -1;
    }
    strncpy(ip,argv[1],strlen(argv[1])+1);   
    port = atoi(argv[2]);
    mode = atoi(argv[3]);
    type = atoi(argv[4]);
    do_login = atoi(argv[5]);
    do_logout = atoi(argv[6]);
    if(argv[7][0] == '0')
    {
        memset(targetname,0,sizeof(TARGET_NAME_MAXLEN));
    }
    else
    {
        strncpy(targetname,argv[7],strlen(argv[7])+1); 
    }
    printf("ip%s,port%d,mode%d,type%d,do_login%d,do_logout%d,tarname%s",ip,port,mode,type,do_login,do_logout,targetname);
