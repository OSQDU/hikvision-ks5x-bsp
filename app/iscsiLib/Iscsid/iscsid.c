...
#include "iface.h"
#include "../include/iscsid.h"
//#include "util/util_thread.h"
#define ISCSID_INITIATOR_NAME_LEN          512
/* global config info */
...
Open-iSCSI initiator daemon.\n\
  -c, --config=[path]     Execute in the config file (" ISCSI_CONFIG_FILE ").\n\
...
static int sync_session(void *data, struct session_info *info)
{
    ...
    if (t->caps & CAP_FW_DB) {
        ...
        if (err) {
            printf("Could not get host no from sid %u. Can not "
    ...
    if (idbm_rec_read(&rec, info->targetname, info->tpgt,
              ...
              &info->iface)) {
        printf("Could not read data from db. Using default and "
...
static void create_new_thread(void)
{
    ...
    {
        printf("set_pthread_attr error\n");
        return ;
    }
    //iRet = pthreadSpawn(NULL, 40/*HD_CTRL_PRIO*/, 2048, iscsi_operate, NULL);
    iRet = pthread_create(&pId,&attr,iscsi_operate,NULL);
    if(0 != iRet)
    {
...
int main(int argc, char *argv[])
{
    ...
    pid_t pid;
    char cInitCurDir[TARGET_NAME_MAXLEN];
    char cConfigCurDir[TARGET_NAME_MAXLEN];
    char *pInitiator_name = NULL;
    

    /*获取路径名*/
    if(        (-1 == GetIpsanCfgDir(cInitCurDir,sizeof(cInitCurDir),INITIATOR_NAME_FILE))   
        || (-1 == GetIpsanCfgDir(cConfigCurDir,sizeof(cConfigCurDir),ISCSI_CONFIG_FILE)) 
    )
    {
        log_close(log_pid);
        exit(1);
    }
    config_file        = cConfigCurDir;
    initiatorname_file = cInitCurDir;
    
    pInitiator_name = (char *)malloc(ISCSID_INITIATOR_NAME_LEN);
    if(NULL == pInitiator_name)
    {
        log_close(log_pid);
        exit(1);
    }
    memset(pInitiator_name , 0 , ISCSID_INITIATOR_NAME_LEN);
    ...
    if (log_daemon) {
        ...
        if (pid < 0) {
            ...
            printf("iSCSI daemon with pid=%d started!", pid);
            exit(0);
        }
        ...
        //IPSAN 单独进程化时需要iscsid打印信息，不能成为守护进程
        #if 0
            daemon_init();
        #endif
    } else {
    ...
    snprintf(pInitiator_name , ISCSID_INITIATOR_NAME_LEN , "iqn.2006.04.fineac.%d.alvin-iscsi-initiator",(int)time(NULL));
    daemon_config.initiator_name = pInitiator_name;
    ...
    if (pid == 0) {
        ...
        printf("Fork failed error %d: existing sessions"
