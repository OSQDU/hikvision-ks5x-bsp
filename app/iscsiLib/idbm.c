...
static void idbm_sync_config(void)
{
	char *config_file;
	FILE *f;
	char cCurDir[TARGET_NAME_MAXLEN];
	
	/*获取路径名*/
	if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),ISCSI_CONFIG_FILE))	
	{
		return;
	}
	      ...
	      char *ip, int port, struct iface_rec *iface)
{
	...
	/*获取路径名*/
	if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),NODE_CONFIG_DIR))	
	{
		return ENOTDIR;
	}
	...
	snprintf(portal, PATH_MAX, "%s/%s/%s,%d", cCurDir,
		 targetname, ip, port);
	log_debug(5, "rec read looking for config file %s.", portal);
	if (!stat(portal, &statb))
		goto read;

	snprintf(portal, PATH_MAX, "%s/%s/%s,%d,%d/%s", cCurDir,
...
static int print_discovered(char *disc_path, int info_level)
{
	...
	for (i = 0; i < n; i++) {
		...
		snprintf(portal, PATH_MAX, "%s/%s/%s,%s,%s/%s", cCurDir,
...
int idbm_print_discovered(discovery_rec_t *drec, int info_level)
{
	...
	case DISCOVERY_TYPE_SENDTARGETS:
		...
		/*获取路径名*/
		if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),ST_CONFIG_DIR))	
		{
			return 0;
		}
		
		snprintf(disc_path, PATH_MAX, "%s/%s,%d", cCurDir,
			 drec->address, drec->port);
		break;
	case DISCOVERY_TYPE_STATIC:

		/*获取路径名*/
		if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),STATIC_CONFIG_DIR))	
		{
			return 0;
		}
		
		snprintf(disc_path, PATH_MAX, "%s", cCurDir);
		break;
	case DISCOVERY_TYPE_ISNS:
		
		/*获取路径名*/
		if(-1 == GetIpsanCfgDir(cCurDir,sizeof(cCurDir),ISNS_CONFIG_DIR))	
		{
			return 0;
		}
...
static int idbm_print_all_st(int info_level)
{
	...
	while ((entity_dent = readdir(entity_dirfd))) {
		...
		if (info_level >= 1) {
			...
			snprintf(disc_dir, PATH_MAX, "%s/%s", cCurDir,
				 entity_dent->d_name);
				...
				char *targetname, int tpgt, char *ip, int port)
{
	...
	snprintf(portal, PATH_MAX, "%s/%s/%s,%d", cCurDir, targetname,
		 ip, port);
	if (stat(portal, &statb)) {
...
read_iface:
	snprintf(portal, PATH_MAX, "%s/%s/%s,%d,%d", cCurDir,
			 ...
			 char *targetname)
{
	...
	snprintf(portal, PATH_MAX, "%s/%s", cCurDir, targetname);
...
idbm_discovery_read(discovery_rec_t *out_rec, char *addr, int port)
{
	...
	snprintf(portal, PATH_MAX, "%s/%s,%d", cCurDir,
		 addr, port);
...
static int idbm_rec_write(node_rec_t *rec)
{
	...
	snprintf(portal, PATH_MAX, "%s", cCurDir);
	...
	snprintf(portal, PATH_MAX, "%s/%s", cCurDir, rec->name);
	if (access(portal, F_OK) != 0) {
		if (mkdir(portal, 0660) != 0) {
			log_error("Could not make %s\n", portal);
			rc = errno;
...
static int setup_disc_to_node_link(char *disc_portal, node_rec_t *rec)
{
	...
	case DISCOVERY_TYPE_STATIC:
		...
		if (access(cCurDir, F_OK) != 0) {
			if (mkdir(cCurDir, 0660) != 0) {
				log_error("Could not make %s\n",
					  cCurDir);
				rc = errno;
			}
...
int idbm_add_node(node_rec_t *newrec, discovery_rec_t *drec, int overwrite)
{
	...
	snprintf(node_portal, PATH_MAX, "%s/%s/%s,%d,%d", cCurDir,
...
{
	/*Modified by mizhoujun at 2014/6/24*/
	/*路径变量*/
	PAL_PATH_T stDir;

	/*初始化*/
	memset((char*)&stDir,0,sizeof(stDir));

	/*获取路径*/
    strncpy((char *)&stDir, ISCSI_DIR_PATH, strlen(ISCSI_DIR_PATH));
	/* make sure root db dir is there */
	if (access(stDir.aPath, F_OK) != 0) {
		if (mkdir(stDir.aPath, 0660) != 0) {
...
/** @fn int GetIpsanCfgDir(char *pcDstDir,size_t dwDstDirSize,const char *pcSrcDir)
* @brief  获取IPSAN配置文件所在目录,并在其后添加子目录名
* @param  pcDstDir     合成后的目录
* @param  dwDstDirSize 目录名的长度
* @param  pcSrcDir     将该目录名追加在IPSAN配置文件所在目录之后。生成新的目录名
* @return 0               
* @return -1 
*/
int GetIpsanCfgDir(char *pcDstDir,size_t uDstDirSize,const char *pcSubDir)
{
	char cCurDir[TARGET_NAME_MAXLEN];
	PAL_PATH_T stDir;		/*路径变量*/

	/*Modified by mizhoujun at 2014/6/24*/

	/*判断入口参数是否有效*/
	if((NULL == pcDstDir)||(NULL == pcSubDir))
	{
	    printf("GetIpsanCfgDir input error!\n");
		return -1;
	}
	...
	/*获取路径*/
    strncpy((char *)&stDir, ISCSI_DIR_PATH, strlen(ISCSI_DIR_PATH));
	/*生成当前路径，路径名长度不能等于缓存大小,等于也默认为超过*/
	if(uDstDirSize == snprintf(pcDstDir, uDstDirSize, "%s%s", stDir.aPath, pcSubDir))
	{
		/*路径名超过最大限制范围,相等也默认超过，即路径名最长为 TARGET_NAME_MAXLEN-1*/
		printf("uDstDirSize error!\n");
		return -1;
	}
	return 0;
}
/*for complie
int pal_get_ipsan_cfg_dir(PAL_PATH_T *pStruStruDir)
{
	;
}
*/
