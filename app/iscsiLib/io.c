...
#ifdef POSIX_TIMER_USED
static void
sigalarm_handler(int signo, siginfo_t *info, void *context)
#else
static void
...
iscsi_io_connect(iscsi_conn_t *conn)
{
    ...
    iSigNo = util_timer_alloc_signo();
    if(util_timer_create(iSigNo, sigalarm_handler, NULL, &timerInfo) != 0)
    {
        printf("fun:%s, timer create failed\n", __func__);
    }
#else
...
#ifdef POSIX_TIMER_USED
    if(util_timer_settime(&timerInfo, TIMER_MODE_SINGLE, conn->login_timeout) != 0)
    {
        printf("set timer time failed in %s\n", __func__);
    }
#else
...
#ifdef POSIX_TIMER_USED
    util_timer_delete(&timerInfo);
    util_timer_free_signo(iSigNo);
#else
	alarm(0);
	       ...
	       int hdr_digest, char *data, int data_digest, int timeout)
{
	...
	if (!ipc) {
		...
		timedout = 0;
#ifdef POSIX_TIMER_USED
        if(util_timer_settime(&timerInfo, TIMER_MODE_SINGLE, timeout) != 0)
        {
            printf("set timer time failed in %s\n", __func__);
        }        
#else
...
iscsi_io_recv_pdu(iscsi_conn_t *conn, struct iscsi_hdr *hdr,
	       ...
	       int timeout)
{
    /*Modified by niechuan 20151127*/
    int iResult = 0;
    struct timeval tempTimeOut;
    memset(&tempTimeOut,0,sizeof(tempTimeOut));
    tempTimeOut.tv_sec = 2;
    
    fd_set rdFd;
    FD_ZERO(&rdFd);
    FD_SET(conn->session->ctrl_fd, &rdFd);
    /*Modified by niechuan 20151127*/
	uint32_t h_bytes = 0;
	...
	do {
        ...
        {
            /*Modified by niechuan 20151127*/
            iResult = select(FD_SETSIZE,&rdFd,(fd_set *)0,(fd_set *)0,&tempTimeOut);
            if (iResult <= 0)
            {
                failed = 1;
                goto done;
            ...
            {
                rlen = recv(session->ctrl_fd,header,sizeof (*hdr) - h_bytes,MSG_DONTWAIT);
            }
            /*Modified by niechuan 20151127*/
        }
		else
