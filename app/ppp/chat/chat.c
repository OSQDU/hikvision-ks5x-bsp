...
#ifndef lint
static const char rcsid[] = "$Id: chat.c,v 1.2 2007/01/20 03:23:50 lzl Exp $";
#endif
...
/*************************by lzl*********************************************/
#define NAME_COUNT 2
#define CONNECT_STRING "HIKVISION"
volatile int  be_connect=0;
char * name_strings[NAME_COUNT]={"ogin:","name:"};
/***********************by lzl**********************************************/
 ...
 */
 	if(be_connect==1&&exit_code == 3){
		be_connect=0;
		msgf("Failed hikvision");
		terminate(0);
 	}
...
register char *string;
{
	...
	*s++ = c;
	...
	/************************add lzl********************************************/
			int myn=0,mylen=0;
			if(be_connect){
			for (myn = 0; myn < NAME_COUNT; ++myn) {
			   if (s - temp >= (mylen = strlen(name_strings[myn]))&&
				strncmp(s - mylen, name_strings[myn], mylen) == 0) {
				if (verbose) {
				if (s > logged)
					msgf("%0.*v", s - logged, logged);
				msgf(" -- got it\n");
