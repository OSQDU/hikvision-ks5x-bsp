...
int xtables_load_ko(const char *modprobe, bool quiet)
{
	...
	if (ret == 0)
		loaded = true;
    #else  /*modify by menghong , 可能导致设备死机, 具体原因待查*/
    fprintf(stderr, "xtables not support load ko!\n");
    ret = -1;
    #endif
...
static int xtables_check_options(const char *name, const struct option *opt)
{
...
int xtables_register_match(struct xtables_match *me)
{
	if (me->version == NULL) {
	...
	if ((me->extra_opts != NULL)
        && (xtables_check_options(me->name, me->extra_opts) != 0))
	{
        return -1;
	}
...
{
	do {
		(void)xtables_register_match(&match[--n]);
	} while (n > 0);
}

int xtables_register_target(struct xtables_target *me)
{
	if (me->version == NULL) {
...
void xtables_deregister_targets(void)
{
    xtables_pending_targets = NULL;
    xtables_targets = NULL;
}
void xtables_deregister_matches(void)
{
